...
int a,count=0,countq=0;
        uint32_t ui32ClockMS;
               ...
               GPIOPinTypeGPIOOutput(GPIO_PORTQ_BASE, GREEN_LED);//|USER_LED2));
                 ...
                 GPIOPinTypeGPIOOutput(GPIO_PORTL_BASE, NEW_LED);
...
if(ui32Key==0)
{
    ...
    countq++; Kentec320x240x16_SSD2119Init(ui32SysClock);
    FrameDraw(&sContext, "Digital Input");
    if(countq>4)
    {
        countq=1;
    }
...
case 1:
          ...
          GrStringDraw(&sContext, "RED LED", -1, 115, 108, 0);
          usprintf(pcBuf, " %03d ", countq);
                    GrStringDrawCentered(&sContext, pcBuf, -1, 80, 100, 1);
                   // a++;
...
case 2:
                                        ...
                                        SysCtlDelay(ui32SysClock/100);
                                                 ...
                                                 GrContextFontSet(&sContext, g_psFontCm32);
                                                 GrStringDraw(&sContext, "GREEN LED", -1, 115, 108, 0);
                                                 usprintf(pcBuf, " %03d ", countq);
a++;GrStringDrawCentered(&sContext, pcBuf, -1, 80, 100, 1);
                                        break ;
...
case 3:
    ui32Key = (ROM_GPIOPinRead(GPIO_PORTE_BASE, GPIO_PIN_5));
                                        ...
                                        usprintf(pcBuf, " %03d ", countq);
                                                 GrContextFontSet(&sContext, g_psFontCm32);
                                                 GrStringDraw(&sContext, "BLUE LED", -1, 115, 108, 0);
                                                 GrStringDrawCentered(&sContext, pcBuf, -1, 80, 100, 1);
a++;
                                        break;
...
case 4:
    ui32Key = (ROM_GPIOPinRead(GPIO_PORTE_BASE, GPIO_PIN_5));
                                        ...
                                        GPIOPinWrite(GPIO_PORTN_BASE, RED_LED, 0x0);
                                        GPIOPinWrite(GPIO_PORTQ_BASE, BLUE_LED,0);
                                        GPIOPinWrite(GPIO_PORTL_BASE, NEW_LED,NEW_LED);
                                        SysCtlDelay(ui32SysClock/100);
                                        usprintf(pcBuf, " %03d ", countq);
                                                 GrContextFontSet(&sContext, g_psFontCm32);
                                                 GrStringDraw(&sContext, "NEW LED", -1, 115, 108, 0);
                                                 GrStringDrawCentered(&sContext, pcBuf, -1, 80, 100, 1);
a++;
                                        break;
