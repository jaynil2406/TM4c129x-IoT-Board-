...
while(1)
{
  ...
  //
   ...
   // ui32Key1 = (ui32Key == 1|ROM_GPIOPinRead(GPIO_PORTP_BASE, GPIO_PIN_1));

    //ui32Key1 = (ROM_GPIOPinRead(GPIO_PORTN_BASE, GPIO_PIN_3) && ROM_GPIOPinRead(GPIO_PORTE_BASE, GPIO_PIN_5));
    ui32Key = (ROM_GPIOPinRead(GPIO_PORTP_BASE, GPIO_PIN_1));
    if((ui32Key ==0))
    {
       // SysCtlDelay(2000 * ui32ClockMS);
        //ui32Key = (ROM_GPIOPinRead(GPIO_PORTN_BASE, GPIO_PIN_3) |ROM_GPIOPinRead(GPIO_PORTE_BASE, GPIO_PIN_5));
        //if(ui32Key == GPIO_PIN_5)
        //{
  //  FrameDraw(&sContext, "UP");
   ...
   // GrContextFontSet(&sContext, g_psFontCm32);
    //GrStringDrawCentered(&sContext, "GPIO = %d", -1,
                //         GrContextDpyWidthGet(&sContext) / 2,
              //           ((GrContextDpyHeightGet(&sContext) - 32) / 2) + 24,
                  //       0);
    //GPIOPinWrite(GPIO_PORTQ_BASE, USER_LED1, 0x0);
